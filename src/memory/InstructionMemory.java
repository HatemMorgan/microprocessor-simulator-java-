...
public class InstructionMemory extends Memory {
	...
	public void placeInstructionsInMemory(String[] instructions) {
		/*
		 * decreaseClockCycles(instructions.length); //Why is this here? Placing
		 * instructions in memory is independent of the clock for (int i = 0; i
		 * < instructions.length; i++) { this.store(getPC(), instructions[i]);
		 * setPC((short) (getPC()+1)); }
		 */
	...
	public static void main(String[] args) {
		Clock c = new Clock();
		InstructionMemory im = new InstructionMemory(2, 10, c,
				writeHitPolicy.writeBack, writeMissPolicy.writeAllocate);
		im.setPC((short) 0);
		String[] instructions = { "LW 1, 2, 0", "ADD 5, 2, 1", "SUB 4, 2, 3",
				"MUL 2, 4, 5" };
		im.placeInstructionsInMemory(instructions); // storing instructions in
													// memory is independent of
													// the
